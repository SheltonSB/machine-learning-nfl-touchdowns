...
# Create FastAPI app
app = FastAPI(
    title="🏈 NFL AI/ML Platform - SQLite Production",
    description="Production NFL platform with SQLite database, real RAG, and ML predictions",
    version="4.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
class PredictionRequest(BaseModel):
    ...
    model_name: Optional[str] = "sqlite_ensemble"
...
async def create_tables():
    """Create SQLite tables for the NFL AI platform"""
...
# Production RAG System (same as before)
...
# Production ML Pipeline (same as before)
class ProductionMLPipeline:
    ...
    async def _train_models(self, data):
        ...
        try:
            ...
            # Save model to file
            try:
                joblib.dump(rf_model, 'models/sqlite_rf_model.pkl')
    ...
    def predict(self, player_id: int, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "model_used": "sqlite_random_forest",
                "reasoning": reasoning,
...
async def health_check():
    return HealthResponse(
        ...
        components={
            "api": "online",
            "sqlite_database": "connected" if DATABASE_AVAILABLE else "disconnected",
...
async def startup_event():
    ...
    logger.info("Starting NFL AI/ML Platform - SQLite Production")
        ...
        if DATABASE_AVAILABLE:
            try:
                ...
                """), {
                    ...
                    "model_used": "sqlite_production_rag",
                    "sources": json.dumps(result.get("sources", [])),
                    "data_freshness": result.get("data_freshness", "unknown")
                })
                db.commit()
                logger.info("RAG query stored in SQLite database")
            except Exception as e:
...
async def create_prediction(prediction_data: PredictionRequest, db = Depends(get_db)):
    """Create prediction using production ML pipeline and store in SQLite"""
    try:
        ...
        if DATABASE_AVAILABLE:
            try:
                ...
                db.commit()
                logger.info("Prediction stored in SQLite database")
            except Exception as e:
...
async def get_production_stats():
    ...
    return {
        "platform": "NFL AI/ML Platform - SQLite Production",
        "version": "4.0.0",
        "status": "live",
        "database": {
            "type": "SQLite",
        ...
        "rag_system": {
            ...
            "data_sources": ["NFL Knowledge Base", "ChromaDB", "SQLite Storage"]
        },
        "ml_pipeline": {
        ...
        "features": {
            "sqlite_database": DATABASE_AVAILABLE,
            "real_rag": True,
            "production_ml": True,
            "live_data": True,
...
async def get_rag_history(db = Depends(get_db)):
    """Get RAG query history from SQLite database"""
    if not DATABASE_AVAILABLE:
        raise HTTPException(status_code=500, detail="Database not available")
